...
| **Lathe** *(mirrored: blaineam/Lathe docs/, generated by `docs/build.py` from `docs/_src/`)* | existing documentation.html · benchmarks.html · mac.html · sami.html | Lathe repo, current `docs/_src/index.html` | owner: "way too much copy". Rewrite `_src/index.html` as a short funnel (hero: open-source Swift + C media engine; primary CTA = GitHub / Swift Package "Get started"; Mac app and Sami as quiet links; 3–5 stories), and **move** (not delete) the ~2,200 words of long copy into documentation.html (API, architecture) or a new `how-it-works.html` added to `PAGES` in `docs/_shared.py`. Run `docs/build.py`; commit with `[ci skip]` (CI builds on push); trigger the mirror. |
